...
import com.example.jobportal.network.RecruiterContactsRepository;
import com.example.jobportal.network.ApiCallback;

public class RecruiterContactSyncService extends Service {
    private static final String TAG = "RecruiterContactSyncService";
    private static final int NOTIFICATION_ID = 2;
    private static final String CHANNEL_ID = "RecruiterContactSyncChannel";
    ...
    public void onCreate() {
        super.onCreate();
        recruiterContactsRepository = new RecruiterContactsRepository(getApplicationContext());
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (!isRunning) {
            isRunning = true;
            startForeground(NOTIFICATION_ID, createNotification("Starting recruiter sync..."));
            syncContacts();
        }
        return START_STICKY;
    }
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(
                CHANNEL_ID,
                "Recruiter Contact Sync Service",
                NotificationManager.IMPORTANCE_LOW
            );
            channel.setDescription("Shows recruiter  sync progress");
    ...
    private Notification createNotification(String message) {
        return new NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Job Portal - Recruiter")

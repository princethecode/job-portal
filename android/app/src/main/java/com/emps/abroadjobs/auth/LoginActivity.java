...
import com.emps.abroadjobs.databinding.ActivityLoginBinding;
...
import com.emps.abroadjobs.network.SafeJsonParser;
...
public class LoginActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // In debug mode, validate API interfaces for proper generic types
        if (BuildConfig.DEBUG) {
            com.emps.abroadjobs.network.RetrofitTestHelper.validateApiServiceMethods();
            com.emps.abroadjobs.network.RetrofitTestHelper.validateRecruiterApiServiceMethods();
        }
    }
                    ...
                    public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                        if (response.isSuccessful()) {
                            try {
                                ...
                                // Use safe parser instead of TypeToken
                                try {
                                    if (SafeJsonParser.isSuccessResponse(responseBody)) {
                                        Log.d(TAG, "FCM token registered successfully after login");
                                    } else {
                                        String errorMessage = SafeJsonParser.extractErrorMessage(responseBody);
